        ...
        if  test_task == TASK_INIT:
            ...
            if ANDROID_LOGCAT_ENABLE == 1:
                ...
                if  result == RESULT_NG:
                    break
            accoff_reason = None
            test_task = TASK_ONOFF

#ADD func for PJ11DEF00-11346 
        elif  test_task == TASK_ONOFF:
            senddata='slog2info'
            q_qnx.put((EV_QNX_SER_WRITE, senddata))
            q_qnx.join()
            for i in range(5):
                acconoffreason=f"initial ACC&IG ON {i+1}/5"
                pika_stop(acconoffreason) 
                time.sleep(10)
                pika_restart(acconoffreason)
                time.sleep(10)
            accoff_reason = None                
